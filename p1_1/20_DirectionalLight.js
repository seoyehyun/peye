...
- Viewing a 3D unit sphere at origin with perspective projection
- Rotating the sphere by ArcBall interface (by left mouse button dragging)
- Keyboard controls:
...
async function initShader() {
    const vertexShaderSource = await readShaderFile('shaders/shVert.glsl');
    const fragmentShaderSource = await readShaderFile('shaders/shFrag.glsl');
...
function render() {
    ...
    sphere.copyVertexNormalsToNormals();
    sphere.updateNormals();

    // drawing the sphere
    shader.use();  // using the sphere's shader
    shader.setInt('u_levels', toonLevels);
